...
const SKILL_DB = {
    ...
    tinko: { id: "tinko", name: "ち⭕️こ", cost: 50, cooldown: 0.1, desc: "相手は0.1秒間タイピング不可" },
    ...
    if (myPartyId) {
        get(ref(db, `parties/${myPartyId}/members`)).then(s => {
            ...
            if(val) {
                ...
                if (equippedSkill === "fundraiser" && !isCustomGame) coinText += " (資金稼ぎ2倍適用!)" or if (equippedSkill === "godfundraiser" && !isCustomGame) coinText += " (神資金稼ぎ4倍適用!)";
